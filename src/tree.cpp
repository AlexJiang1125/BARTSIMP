...
//--------------------
// node id
// class object is a pointer to the current node it's at??
// it is the absolute id. i.e you start from the top
...
//get all nodes
/*
 * v is possibly an empty vector of tree node
 * class objects??
 */
void tree::getnodes(npv& v)
{
  v.push_back(this);
  // if not bottom node, go deeper
  if(l) {
    l->getnodes(v);
    r->getnodes(v);
  }
...
{
  // get the pointer to the desired node by id
...
bool tree::death(size_t nid, double theta)
{
  ...
  // if no such node, reports error
  if(nb==0) {
    cout << "error in death, nid invalid\n";
    return false;
  }
  // is nog node, can delete
...
//delete the nodes from bottom to top
void tree::tonull()
{
  size_t ts = treesize();
  //loop invariant: ts>=1
...
void tree::printtreeToFile(xinfo& xi)
{
  std::ofstream MyFile("/Users/alexziyujiang/Documents/data/SBART/results.txt", MyFile.out | MyFile.app);
  tree::cnpv nds;
  this->getnodes(nds);
  MyFile << "==== Tree ====" << endl;
  MyFile << nds.size() << std::endl;
  for(size_t i=0;i<nds.size();i++) {
    MyFile << nds[i]->nid() << " ";
    MyFile << nds[i]->getv() << " ";
    MyFile << nds[i]->getc() << " ";
    MyFile << xi[nds[i]->getv()][nds[i]->getc()] << " ";
    MyFile << nds[i]->gettheta() << endl;
  }
  MyFile.close();
  return;
}
...
// **only used for testings purpose! initialize the tree structure to be what you want basically
void tree::initialize(size_t m) {
  //this->tonull();
  //this->birth(1,0,50,2.0/m,0);
  //this->birth(3,1,30,-2.0/m,0);
}
